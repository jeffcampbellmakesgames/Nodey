...
namespace JCMG.Nodey
{
	...
	public class NodePort
	{
		...
		public bool IsInput
		{
			get { return direction == PortDirectionType.Input; }
		}

		public bool IsOutput
		{
			get { return direction == PortDirectionType.Output; }
		}
		...
		public Type ValueType
		{
			...
			set
			{
				...
				if (value != null)
				{
					_typeQualifiedName = value.AssemblyQualifiedName;
				}
			}
		}
		...
		[SerializeField]
		private PortDirectionType _direction;
		...
		public NodePort(FieldInfo fieldInfo)
		{
			...
			for (var i = 0; i < attribs.Length; i++)
			{
				...
				{
					_direction = PortDirectionType.Input;
				...
				{
					_direction = PortDirectionType.Output;
		...
		///     ports.
		/// </summary>
		public NodePort(string fieldName, Type type, PortDirectionType direction, ConnectionType connectionType,
		                TypeConstraint typeConstraint, Node node)
		{
		...
		public void Connect(NodePort port)
		{
			...
			{
				Debug.LogWarning(
					"Cannot connect two " + (direction == PortDirectionType.Input ? "input" : "output") + " connections");
				return;
			}
			#if UNITY_EDITOR
		...
		public bool CanConnectTo(NodePort port)
		{
			...
			if (input.typeConstraint == TypeConstraint.Inherited && !input.ValueType.IsAssignableFrom(output.ValueType))
			{
				return false;
			}

			if (input.typeConstraint == TypeConstraint.Strict && input.ValueType != output.ValueType)
			{
				return false;
			}

			if (input.typeConstraint == TypeConstraint.InheritedInverse &&
			...
			if (output.typeConstraint == TypeConstraint.Inherited && !input.ValueType.IsAssignableFrom(output.ValueType))
			{
				return false;
			}

			if (output.typeConstraint == TypeConstraint.Strict && input.ValueType != output.ValueType)
			{
				return false;
			}

			if (output.typeConstraint == TypeConstraint.InheritedInverse &&
			    !output.ValueType.IsAssignableFrom(input.ValueType))
			{
				return false;
			}
