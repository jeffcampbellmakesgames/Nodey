...
namespace JCMG.Nodey.Editor
{
	public partial class NodeEditorWindow
	{
		...
		[NonSerialized]
		private NodePort autoConnectOutput;
		...
		[NonSerialized]
		public NodePort hoveredPort = null;
		...
		public static NodeActivityType currentActivityType = NodeActivityType.Idle;
			...
			switch (e.type)
			{
				...
				case EventType.MouseDrag:
					if (e.button == 0)
					{
						...
						else if (currentActivityType == NodeActivityType.HoldNode)
						{
							RecalculateDragOffsets(e);
							currentActivityType = NodeActivityType.DragNode;
							Repaint();
						}

						if (currentActivityType == NodeActivityType.DragNode)
						{
						...
						else if (currentActivityType == NodeActivityType.HoldGrid)
						{
							currentActivityType = NodeActivityType.DragGrid;
							preBoxSelection = Selection.objects;
							preBoxSelectionReroute = selectedReroutes.ToArray();
							dragBoxStart = WindowToGridPosition(e.mousePosition);
							Repaint();
						}
						else if (currentActivityType == NodeActivityType.DragGrid)
						{
				...
				case EventType.MouseDown:
					...
					if (e.button == 0)
					{
						...
						else if (IsHoveringNode && IsHoveringTitle(hoveredNode))
						{
							...
							e.Use();
							currentActivityType = NodeActivityType.HoldNode;
						}
						...
						{
							currentActivityType = NodeActivityType.HoldGrid;
							if (!e.control && !e.shift)
							{
								selectedReroutes.Clear();
								Selection.activeObject = null;
					...
					if (e.button == 0)
					{
						...
						if (IsDraggingPort)
						{
							...
							// Open context menu for auto-connection if there is no target node
							         ...
							         NodeEditorPreferences.GetSettings().dragToCreate &&
							         autoConnectOutput != null)
							{
						...
						else if (currentActivityType == NodeActivityType.DragNode)
						{
						...
						if (currentActivityType == NodeActivityType.HoldNode && !(e.control || e.shift))
						{
							selectedReroutes.Clear();
							...
							if (isDoubleClick)
							{
								var nodeDimension = nodeSizes.ContainsKey(hoveredNode)
						...
						Repaint();
						currentActivityType = NodeActivityType.Idle;
					}
					else if (e.button == 1 || e.button == 2)
					{
						if (!isPanning)
				...
				case EventType.Ignore:
					...
					if (e.rawType == EventType.MouseUp && currentActivityType == NodeActivityType.DragGrid)
					{
						Repaint();
						currentActivityType = NodeActivityType.Idle;
					}
		...
		public void Home()
		{
			...
			if (nodes.Count > 0)
			{
				var minPos = nodes.Select(x => x.position)
					.Aggregate((x, y) => new Vector2(Mathf.Min(x.x, y.x), Mathf.Min(x.y, y.y)));
				var maxPos = nodes.Select(x => x.position + (nodeSizes.ContainsKey(x) ? nodeSizes[x] : Vector2.zero))
		...
		public void DuplicateSelectedNodes()
		{
			...
			var selectedNodes = Selection.objects.Select(x => x as Node).Where(x => x != null && x.graph == graph).ToArray();
			...
			var topLeftNode = selectedNodes.Select(x => x.position)
				.Aggregate((x, y) => new Vector2(Mathf.Min(x.x, y.x), Mathf.Min(x.y, y.y)));
		...
		private void InsertDuplicateNodes(Node[] nodes, Vector2 topLeft)
		{
			...
			// Get top-left node
			var topLeftNode = nodes.Select(x => x.position)
				.Aggregate((x, y) => new Vector2(Mathf.Min(x.x, y.x), Mathf.Min(x.y, y.y)));
			...
			for (var i = 0; i < nodes.Length; i++)
			{
				...
				foreach (var port in srcNode.Ports)
				{
					...
					{
						var inputPort = port.direction == PortDirectionType.Input ? port : port.GetConnection(c);
						var outputPort = port.direction == PortDirectionType.Output ? port : port.GetConnection(c);
		...
		public void DrawDraggedConnection()
		{
			if (IsDraggingPort)
			{
				...
				if (!portConnectionPoints.TryGetValue(draggedOutput, out var fromRect))
				{
					return;
				}
		...
		public void AutoConnect(Node node)
		{
			...
			var inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
