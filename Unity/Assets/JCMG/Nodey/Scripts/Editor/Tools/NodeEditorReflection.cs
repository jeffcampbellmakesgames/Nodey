...
{
	/// <summary> Contains reflection-related extensions </summary>
	public static class NodeEditorReflection
	{
		...
		[NonSerialized]
		private static Dictionary<Type, Color> nodeTint;
		...
		public static bool TryGetAttributeTint(this Type nodeType, out Color tint)
		{
			if (nodeTint == null)
			{
				CacheAttributes<Color, NodeTintAttribute>(ref nodeTint, x => x.color);
			}
		...
		public static bool TryGetAttributeWidth(this Type nodeType, out int width)
		{
			if (nodeWidth == null)
			{
				CacheAttributes<int, NodeWidthAttribute>(ref nodeWidth, x => x.width);
			}

			return nodeWidth.TryGetValue(nodeType, out width);
		}

		private static void CacheAttributes<V, A>(ref Dictionary<Type, V> dict, Func<A, V> getter)
			where A : Attribute
		{
		...
		public static void OpenPreferences()
		{
			try
			{
                ...
                if ((bool) refreshField.GetValue(window))
				{
                    type.GetMethod(
						"AddCustomSections",
						BindingFlags.NonPublic | BindingFlags.Instance).Invoke(window, null);
                ...
                for (int i = 0; i < sections.Count; i++)
				{
                    ...
                    if (sectionContent.text == "Node Editor")
					{
                        ...
                        FieldInfo sectionIndexField = type.GetField(
							"m_SelectedSectionIndex",
			...
			catch (Exception e)
			{
				Debug.LogError(e);
				Debug.LogWarning(
					"Unity has changed around internally. Can't open properties through reflection. Please contact Nodey " +
					"developer and supply unity version number.");
			}
		}
	}
}
